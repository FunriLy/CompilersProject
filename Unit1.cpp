	...
	typedef enum  { NUL, IDENT, NUMBER, PLUS, MINUS, TIMES,
		            ...
		            // 实验内容：增加 else、for 等保留字 其中 do 已经存在了
			...
			if(SYM!=IDENT) {
				...
				if(SYM!=BECOMES) {
					...
					if (SYM!=STEPSYM) {
						...
						if(SYM!=UNTILSYM) {
							Error(19); // 没有读取到 until
						} else {
							CODE[CX1-1].A=CX; // 如果是赋值语句，跳转到UNTILSYM执行
							GetSym();
							EXPRESSION(SymSetUnion(SymSetNew(UNTILSYM, STEPSYM, DOSYM), FSYS), LEV, TX);
							GEN(LOD, LEV-TABLE[i].vp.LEVEL, TABLE[i].vp.ADR);
							GEN(OPR, 0, 12);
	...
	void __fastcall TForm1::ButtonRunClick(TObject *Sender) {
	  ...
	  strcpy(KWORD[ 3], "CALL");	strcpy(KWORD[ 4],"CONST");
	  strcpy(KWORD[ 5], "DEC"); // 课程设计 --
	  strcpy(KWORD[ 6], "DIVEQUAL");strcpy(KWORD[ 7],"DO");
	  strcpy(KWORD[ 8], "ELSE");	strcpy(KWORD[ 9],"END");
	  strcpy(KWORD[10], "FOR");		strcpy(KWORD[11], "GATE");
	  strcpy(KWORD[12], "INC"); // 课程设计 ++
	  strcpy(KWORD[13], "IF");		strcpy(KWORD[14], "MULEQUAL");
	  strcpy(KWORD[15], "ODD");     strcpy(KWORD[16], "OR");
	  strcpy(KWORD[17],"PROCEDURE");strcpy(KWORD[18],"PROGRAM");
	  strcpy(KWORD[19], "READ");	strcpy(KWORD[20], "RETURN");
	  strcpy(KWORD[21], "STEP");	strcpy(KWORD[22],"THEN");
	  ...
	  WSYM[ 5]=DECSYM;
	  WSYM[ 6]=DIVEQUALSYM;	WSYM[ 7]=DOSYM;
	  WSYM[ 8]=ELSESYM;		WSYM[ 9]=ENDSYM;
	  WSYM[10]=FORSYM;		WSYM[11]=GATESYM;
	  WSYM[12]=INCSYM;
	  WSYM[13]=IFSYM;		WSYM[14]=MULEQUALSYM;
	  WSYM[15]=ODDSYM;		WSYM[16]=ORSYM;
	  WSYM[17]=PROCSYM;		WSYM[18]=PROGSYM;
	  WSYM[19]=READSYM;		WSYM[20]=RETURNSYM;
	  WSYM[21]=STEPSYM;		WSYM[22]=THENSYM;
